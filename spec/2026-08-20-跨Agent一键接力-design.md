# Codex 与 Claude 桌面端一键接力

日期：2026-08-20

## 目标

在展开态的近期任务中，由用户手动把一个已经停止的 Codex 任务交给 Claude 桌面端中的 Code 会话，或把
一个已经停止的 Claude Code 会话交给 Codex。接力恢复的是可验证的任务现场，不复制模型
...
- 展开态“近期任务/活跃会话”每行悬停时出现“接力图标 + 目标 Agent 文字”的胶囊按钮，
  不使用只有图标、依赖悬停提示才能理解的入口。
- 按钮内容顺序固定为左侧文字、右侧接力图标。
- Codex 行固定显示“交给 Claude”，Claude 行固定显示“交给 Codex”，两边能力和状态反馈一致，
  不增加选择弹窗。Codex 侧的 Claude 目标仍指 Claude 桌面端中的 Code 会话。
...
- 生成中显示进度；成功显示“已交给 …”；降级时显示指令已复制；失败显示原因。
...
- `AgentTaskReference`：来源、会话 ID、标题、目录和记录位置。
...
4. 启动降级：Claude 桌面端未安装、app-server 失败或深链失效时，交接包仍存在且指令进入剪贴板。
5. 临时仓库手动完成双向接力，核对目标、待办、修改文件和工作目录。
...
8. 在有两条近期任务的列表上反复移入、移出接力按钮，确认任务标题、列表顶部和底部均没有上下位移。
